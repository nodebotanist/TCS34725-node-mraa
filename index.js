...
setInterval(() => {
    ...
    clear = colorSensor.readReg(0x94) << 4
    clear |= colorSensor.readReg(0x95)
    clearGraph.update(clear)

    // red channel-- low byte 0x16, high byte 0x17
    red = colorSensor.readReg(0x96) << 4
    red |= colorSensor.readReg(0x97)
    red = Math.floor((red / clear) * 256)
    redGraph.update(red)

    // green channel-- low byte 0x18, high byte 0x19
    green = colorSensor.readReg(0x18) << 4
    green |= colorSensor.readReg(0x19)
    green = Math.floor((green / clear) * 256)
    greenGraph.update(green)

    // blue channel-- low byte 0x1A, high byte 0x1B
    blue = colorSensor.readReg(0x1A) << 4
    blue |= colorSensor.readReg(0x1B)
